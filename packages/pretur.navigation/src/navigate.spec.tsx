...
import { Navigate, NavigatePassedProps } from '../src/Navigate';

const component: React.StatelessComponent<NavigatePassedProps<number>> = props => {
...
describe('Navigate', () => {
  ...
  it('should render nothing when no page is open', () => {
    ...
    renderer.render(<Navigate navigator={navigator} />);
  ...
  it('should render the active page and pass down the expecetd props', () => {
    ...
    const renderer = createRenderer();
    renderer.render(<Navigate navigator={nav} />);
    const output = renderer.getRenderOutput<React.ReactElement<NavigatePassedProps<number>>>();
